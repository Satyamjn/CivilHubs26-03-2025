...
const LatestNews = () => {
  ...
  return (
    <div className="py-10 bg-auto -mt-14">
      <h2 className="text-3xl ml-16 font-bold text-start ">
        Latest <span className="text-teal-500">Blog</span>
      </h2>
      <div className="w-full max-w-screen-1xl px-4 sm:px-6 lg:px-8 mx-auto relative"> {/* Changed max-w-screen-3xl to max-w-screen-xl */}
        <Slider {...settings} className="w-full">
          {newsData.map((news, index) => (
            ...
            >
              <div 
                className="bg-white p-6 rounded-lg shadow-lg transition-transform duration-300 hover:shadow-md hover:shadow-teal-500 hover:cursor-pointer z-20 h-max w-full max-w-xs sm:max-w-sm md:max-w-md mx-auto" 
                onClick={() => navigate('/blogdetail1C', { state: { news } })} // Navigate to the blog detail page
              >
                <img
                  src={news.imageUrl}
                  alt={news.title}
